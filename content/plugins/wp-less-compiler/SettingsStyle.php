...
class SettingsStyle extends PluginObject {
	...
	public function become_array() {
		...
		$css["path"]  = isset($this->css->path) ? $this->_strip_template_path( $this->css->path ) : null;
		$css["hash"]  = isset($this->css->hash) ? $this->css->hash : null;
	
		$less["url"]  = isset($this->less->url) ? $this->less->url : null;
		$less["path"] = isset($this->less->path) ? $this->_strip_template_path( $this->less->path ) : null;
	...
	private function _create_from_object( $object ) {
		...
		$css["path"]  = isset($asset->css->path) ? $this->_strip_template_path( $asset->css->path  ): null;
		$css["hash"]  = isset($asset->css->hash) ? $asset->css->hash : null;
	
		$less["url"]  = isset($asset->less->url) ? $asset->less->url : null;
		$less["path"] = isset($asset->less->path) ? $this->_strip_template_path( $asset->less->path ) : null;
	...
	private function _create_from_array( $array ) {
		...
		$css["path"]  = isset($array["css"]["path"]) ? $this->_strip_template_path( $array["css"]["path"] ) : null;
		$css["hash"]  = isset($array["css"]["hash"]) ? $array["css"]["hash"] : null;
	
		$less["url"]  = isset($array["less"]["url"]) ? $array["less"]["url"] : null;
		$less["path"] = isset($array["less"]["path"]) ? $this->_strip_template_path( $array["less"]["path"] ) : null;

		return array( "css" => $css, "less" => $less );
	}

	private function _strip_template_path( $path ) {
		return str_replace( get_template_directory(), "", $path );
	}
